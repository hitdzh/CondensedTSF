...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        if self.timeenc == 0:
            ...
            # Add minute feature for sub-hourly data
            if self.freq in ['10min', '15min', '30min', '5min', 't']:
                df_stamp['minute'] = df_stamp.date.dt.minute
                if self.freq == '10min':
                    df_stamp['minute'] = (df_stamp['minute'] // 10).astype(int)
                elif self.freq == '15min':
                    df_stamp['minute'] = (df_stamp['minute'] // 15).astype(int)
                elif self.freq == '30min':
                    df_stamp['minute'] = (df_stamp['minute'] // 30).astype(int)
                elif self.freq == '5min':
                    df_stamp['minute'] = (df_stamp['minute'] // 5).astype(int)
                elif self.freq == 't':
                    df_stamp['minute'] = (df_stamp['minute'] // 15).astype(int)
